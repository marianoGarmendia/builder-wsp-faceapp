...
const extractMimeTypeFromCtx = (ctx: any): string | undefined => {
  ...
  while (m && depth < 5) {
    ...
    m = m.ephemeralMessage?.message || m.viewOnceMessageV2?.message || m.viewOnceMessage?.message || m.deviceSentMessage?.message;
    depth++;
  }
  return undefined;
};
...
const saveIncomingFile = async (ctx: any, provider: any, baseDir: string): Promise<string> => {
  try {
...
const sendFileToEndpoint = async (
  ...
  try {
    ...
    if (!endpointConfirm) {
      console.warn("sendFileToEndpoint: endpointConfirm ausente en captación", rec);
      return false;
    }
...
const mediaFlowGpt = addKeyword<Provider, MemoryDB>(utils.setEvent("MEDIA_FLOW_GPT"))  // o tu keyword
  ...
  .addAction(async (ctx, { provider, flowDynamic }) => {
    ...
    console.log("✅ Recibido y guardado: *${safeName}* (${mimeType || 'desconocido'})");
...
const processResponseWithDocsAndMedia = async (ctx: CtxIncomingMessage, { flowDynamic, state, provider }) => {
...
const confirmFlow = addKeyword<Provider, MemoryDB>(utils.setEvent("CONFIRM_FLOW")).addAction(async (ctx, { flowDynamic, state, provider }) => {
  const userId = ctx.from; // Use the user's ID to create a unique queue for each user
  ...
  if (!recConfirm) {
    console.log("confirmFlow: usuario sin captación activa. Se ignora el mensaje.");
    return;
  }
...
const mediaFlowCursor = addKeyword<Provider, MemoryDB>(EVENTS.MEDIA)
    .addAction( { capture: false }, async (ctx, { provider, state, flowDynamic }) => {
        ...
        if (!recMedia) {
          console.log("[MEDIA][IGNORADO] from:", (ctx as any)?.from, "keys:", messageKeys, "mime:", mimeFromCtx, "body:", (ctx as any)?.body)
          // return;
        }
        console.log("[MEDIA] keys:", messageKeys, "mime:", mimeFromCtx, "body:", (ctx as any)?.body)
        ...
        if (recMedia) {
          const ok = await sendFileToEndpoint(savedPath, ctx as any, recMedia, mimeFromCtx, "media")
          if (ok) {
...
const documentFlow = addKeyword<Provider, MemoryDB>(EVENTS.DOCUMENT)
    .addAction( { capture: false }, async (ctx, { provider, state, flowDynamic }) => {
        const messageKeys = Object.keys((ctx as any)?.message || {})
        const mimeFromCtx = extractMimeTypeFromCtx(ctx as any)
        const m: any = (ctx as any)?.message || {}
        const docName = m?.documentMessage?.fileName || m?.documentMessage?.title
        const docSize = m?.documentMessage?.fileLength?.toString?.() || m?.documentMessage?.fileLength
        // Guard: validar captación antes de procesar documentos, pero loguear para debug
        const recDoc = getCaptacion((ctx as any)?.from || "");
        if (!recDoc) {
          console.log("[DOCUMENT][IGNORADO] from:", (ctx as any)?.from, "name:", docName, "size:", docSize, "mime:", mimeFromCtx, "keys:", messageKeys, "body:", (ctx as any)?.body)
          // return;
        }
        console.log("[DOCUMENT] keys:", messageKeys, "name:", docName, "size:", docSize, "mime:", mimeFromCtx, "body:", (ctx as any)?.body)
        const savedPath = await saveIncomingFile(ctx, provider, MEDIA_DIR)
        await state.update({ lastDocumentPath: savedPath })
        console.log("✅ Documento guardado DOCUMENT, path: ", savedPath)
        // Si hay captación, enviar al endpoint de confirmación
        if (recDoc) {
          const ok = await sendFileToEndpoint(savedPath, ctx as any, recDoc, mimeFromCtx, "document")
          if (ok) {
            await flowDynamic([{ body: "Tu archivo ha sido registrado" }])
          } else {
...
const welcomeFlow = addKeyword<Provider, MemoryDB>(EVENTS.WELCOME).addAction(
  ...
  console.log("ctx", ctx);
    ...
    if (!recWelcome) {
      console.log("welcomeFlow: usuario sin captación activa. Mensaje ignorado.");
      return;
    }
